corr <- function(directory, threshold = 0) {
  ...
  for(i in seq(filename)) {
    #creating a for loop for a sequential filename. 
    airquality <- read.csv(filename[i])
    #Reading the filename csv file to look up individual files.
    cases <- complete.cases(airquality)
    #Counts the total number of cases that are not NA.
    airquality <- airquality[cases, ]
    #filters back from boolean to dataframe
